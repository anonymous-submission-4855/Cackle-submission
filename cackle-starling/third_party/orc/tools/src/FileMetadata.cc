...
class S3FileInputStream : public orc::InputStream{
  ...
  public:
    ...
    void read(void* buf, uint64_t length, uint64_t offset) override{
      std::cerr << "reading s3 file from idx " << offset << " for " << length << "bytes" << std::endl;
      Aws::S3::Model::GetObjectRequest go_req;
      go_req.SetBucket(bucket);
      go_req.SetKey(key);
      go_req.SetRange("bytes="+std::to_string(offset)+"-"+std::to_string(offset+length));
      ...
      go_res.GetResult().GetBody().read(reinterpret_cast<char*>(buf), length);
    }
...
void printMetadata(std::ostream & out, const char*filename, bool verbose) {
  ...
  std::unique_ptr<orc::Reader> reader = orc::createReader(std::unique_ptr<orc::InputStream>(new S3FileInputStream("mit-lambda-networking", "data/"+std::string(filename))), orc::ReaderOptions());
  out << "{ \"name\": \"" << filename << "\",\n";
